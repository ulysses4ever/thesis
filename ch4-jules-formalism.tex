...
\begin{figure}[h]
  \makebox[14cm][c]{
  \includegraphics[width=1.5\columnwidth]{figs/compile.pdf}
  }
...
The syntax of \jules methods is defined in~\figref{syntax}. I use two key
...
criteria below (\ssecref{wellform}) should rule out \emph{wrong} programs statically, but
...
\end{mathpar}
  ...
  a direct method invocation in the extended table (middle-right)}}\label{compile}
...
Next, let us review the definition from~\figref{fig:max-devirt} in more details.
...
\begin{lemma}[Preserving Maximal Devirtualization]\label{lem:max-devirt}
  For any well-for\-med method table \mtbl, register typing \ol\ty,
...
Putting it all together, I have shown that type-grounded methods
...
\PAPERVERSION{}{
In what follows, I give a formal definition of
...
\MODIFY{
  \begin{definition}[Context-Insensitive Type Inference%
    \protect\footnote{%
      It may look strange to apply the term \emph{context-insensitive}
      to an interface of a static analysis rather than to a concrete analysis.
      But the requirement I define here limits possible implementations of type inference in
      a way that would not allow a context-sensitive analysis, and this is the reason I use
      the term.
      The reason I believe that context-sensitive analysis cannot satisfy the requirement is because
      the requirement does not allow to distinguish call sites of methods.
    }%
]
...
\ADD{
In this section, I prove that evaluating a program with just-in-time
compilation yields the same result as if no compilation occurred. I use two
instantiations of the \jules semantics, one (written~\stepd) where
\jit is the identity, and the other (written \stepj) where
\jit is defined as before. The proof strategy is to
\begin{enumerate}
...
As I show below, when run with the dispatch semantics, related configurations
...
Next, I show that compilation as defined in \figref{fig:compile-wd}
...
% In this chapter I build a model of Juila's JIT,
% present formal definitions of type groundedness and stability,
% and a proof that
