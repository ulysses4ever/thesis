...
corner case 3 in \ssecref{ssect:uconstr-types}),
...
\begin{enumerate}
  ...
  %First challenge with using a sample of concrete types is to choose
  \item
...
corner case 3 (\ssecref{ssect:uconstr-types}).
...
packages, including fully instantiated parametric types.
  ...
  %Second challenge with using a sample of concrete types is to choose
  \item
  \emph{At which moment start using the database?} %instead of the basic enumeration approach.
...
parameters: the amount of fuel to use to discover concrete types, and whether to
...
\begin{enumerate}
  ...
  \item The method is type stable against all known concrete types in the current session
  or, if the type database is used for underconstrained types,
  against a subset of all known types defined
  %via substitution of types coming
  %
  as follows. The concrete types are collected with the
  enumeration procedure when substituting types coming from the type database
  for the \c{Any} type.

  The outcome can also be reached via running type inference with abstract types
  (\ssecref{ssec:approx:space:abstract}).
  ...
  \item
  Cannot decide type stability because the algorithm ran out of fuel
  while running the enumeration (no matter whether it was enhanced with a type
  database or not).
\end{enumerate}
...
In \chapref{chap:empirical}, I describe a dynamic type stability analysis of a
...
proposed in this chapter, I run my implementation of the algorithm%
...
just over the number of unstable methods identified by tracing (28\%).
...
% , at
% least, for the packages which we know to have issues with type stability.
